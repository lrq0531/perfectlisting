...
export async function describeImageMock() {
  return {
    description: 'A handcrafted ceramic coffee mug with a matte blue glaze and rounded handle.',
  };
}
    ...
    messages: [
      {
        role: 'system',
        content:
          'You are an AI assistant that writes compelling e-commerce listings from product images.',
      },
      {
        role: 'user',
        content: [
...
export async function generateListingFromPrompt({ product, description, platform, imageText }) {
  ...
  const res = await client.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `You are an expert e-commerce copywriter specializing in ${platform}.`,
      },
